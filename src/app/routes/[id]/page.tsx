...
import { 
  ...
  Activity, ClipboardCheck, ChevronUp, ChevronDown, Users2, RotateCcw, ShieldAlert
} from "lucide-react"
  ...
  <div className="space-y-1.5 mb-4">
    ...
    <div className="border border-black rounded-lg overflow-hidden shadow-sm bg-white">
      <ScrollArea className="w-full">
        <Table className="text-[9px] min-w-max uppercase">
            ...
            <div className="divide-y">
              {filteredSuppliers.map(s => (
                <div key={s.id} onClick={() => setSelectedSupplier(s)} className={cn("p-3 cursor-pointer hover:bg-primary/5 transition-all", selectedSupplier?.id === s.id ? 'bg-primary/5 border-l-4 border-primary' : '')}>
                  ...
                  <div className="flex items-center gap-2 mt-1"><Badge variant="outline" className="h-4 px-1.5 text-[8px] font-black border-none bg-muted/50">ID: {s.supplierId}</Badge><span className="text-[9px] text-muted-foreground font-bold uppercase">{s.address}</span></div>
                </div>
              ))}
            </div>
          </ScrollArea>
          ...
          {selectedSupplier ? (
            <ScrollArea className="w-full h-full">
              <div className="p-8 printable-report bg-white text-left max-w-[210mm] mx-auto border-[1.5px] border-black m-4 shadow-xl rounded-sm">
                ...
                <div className="w-full border-b-[4px] border-black pb-3 mb-6 text-center"><h3 className="text-[20pt] font-black uppercase text-primary">{selectedSupplier.name}</h3><p className="text-[11pt] font-black text-muted-foreground uppercase tracking-widest">ID: {selectedSupplier.supplierId} | सविस्तर अहवाल</p></div>
                ...
                <div className="grid grid-cols-2 gap-10 text-[12px] font-bold">
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-col"><span>पत्ता</span><span className="font-medium text-slate-600 leading-tight">{selectedSupplier.address || "-"}</span></div>
                    </div>
                  </div>
                  <div className="space-y-4">
            ...
            <div className="flex flex-col items-center justify-center h-full opacity-20 p-20 text-center"><Building2 className="h-16 w-16 mb-4" /><h4 className="font-black uppercase tracking-[0.4em] text-sm">सप्लायर निवडा</h4></div>
          )}
        </Card>
      </div>
